...
- Added ``UNION ALL`` :ref:`operator <glossary-operator>` to produce the
  combined result of two or more queries, e.g.:
...
- Added support to manually control the :ref:`allocation of shards
  <glossary-shard-allocation>` using ``ALTER TABLE REROUTE``. Supported
